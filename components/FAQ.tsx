          ...
          <div className={`pl-12 transition-all duration-500 ${
            ...
            {index === 0 && (
              <div className="mt-4 flex items-center text-sm text-[#0D6EFD] hover:text-blue-700 transition-colors">
                ...
                <a href="#" className="hover:underline">Посмотреть пример отчета</a>
              </div>
            )}
            ...
            {index === 1 && (
              <div className="mt-4 flex items-center text-sm text-[#0D6EFD] hover:text-blue-700 transition-colors">
                ...
                <a href="#" className="hover:underline">Способы вывода средств</a>
              </div>
            )}
          </div>
        </div>
  ...
  const faqData = [
    {
      ...
      answer: 'Каждый партнер получает уникальную реферальную ссылку. Когда пользователь переходит по ней, мы устанавливаем cookie в его браузере на 90 дней. Все регистрации и оплаты, совершенные в течение этого периода, будут засчитаны вам. В личном кабинете вы можете отслеживать каждый переход и конверсию в режиме реального времени.',
    },
    {
      question: 'Как и когда я смогу вывести деньги?',
      answer: 'Выплаты производятся раз в месяц, в начале каждого календарного месяца, при достижении минимальной суммы в $50. Мы поддерживаем несколько способов вывода: банковский перевод, PayPal, Wise, а также популярные электронные кошельки. Обработка запроса на вывод занимает до 3 рабочих дней.',
    },
    {
      question: 'Какие промо-материалы вы предоставляете?',
      answer: 'В личном кабинете партнера вы найдете обширную библиотеку готовых материалов: баннеры разных размеров (от 300x250 до 1200x628), адаптированные для различных платформ, готовые тексты для постов в социальных сетях, email-шаблоны для рассылок, презентации продукта, кейсы успешных клиентов и видео-обзоры. Все материалы регулярно обновляются.',
    },
    {
      question: 'Есть ли какие-то ограничения?',
      answer: 'Да, у нас есть несколько важных правил: запрещено использовать спам-методы для продвижения, нельзя регистрировать собственные аккаунты по своей реферальной ссылке, запрещена контекстная реклама с прямым упоминанием бренда Revvy без согласования. Также не допускается введение пользователей в заблуждение относительно функций продукта. Полный список правил доступен в личном кабинете.',
    },
    {
      question: 'Могу ли я отслеживать эффективность разных каналов?',
